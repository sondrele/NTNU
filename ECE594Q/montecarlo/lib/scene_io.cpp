...
#define IO_CHECK(nE,nA)	if ((nE) != (nA)) { \
...
read_cameraA(SceneIO *scene, FILE *fp)
{
  ...
  fscanf(fp," {");
  IO_CHECK(3, fscanf(fp," position %g %g %g", &camera->position[0],
		  &camera->position[1], &camera->position[2]));
  IO_CHECK(3, fscanf(fp," viewDirection %g %g %g", &camera->viewDirection[0],
		  &camera->viewDirection[1], &camera->viewDirection[2]));
  IO_CHECK(1, fscanf(fp," focalDistance %g", &camera->focalDistance));
  IO_CHECK(3, fscanf(fp," orthoUp %g %g %g", &camera->orthoUp[0],
		  &camera->orthoUp[1], &camera->orthoUp[2]));
  IO_CHECK(1, fscanf(fp," verticalFOV %g", &camera->verticalFOV));
  fscanf(fp," }");
}
...
read_point_lightA(SceneIO *scene, FILE *fp)
{
  ...
  fscanf(fp," {");
  IO_CHECK(3, fscanf(fp," position %g %g %g", &light->position[0],
		  &light->position[1], &light->position[2]));
...
read_directional_lightA(SceneIO *scene, FILE *fp)
{
  ...
  fscanf(fp," {");
  IO_CHECK(3, fscanf(fp," direction %g %g %g", &light->direction[0],
		  &light->direction[1], &light->direction[2]));
  IO_CHECK(3, fscanf(fp," color %g %g %g", &light->color[0],
		  &light->color[1], &light->color[2]));
  fscanf(fp," }");
...
read_objectA(ObjIO *obj, FILE *fp)
{
  ...
  IO_CHECK(1,fscanf(fp," numMaterials %ld",&obj->numMaterials));
...
{
  fscanf(fp," material {");
  IO_CHECK(3, fscanf(fp," diffColor %g %g %g", &material->diffColor[0],
		  &material->diffColor[1], &material->diffColor[2]));
  IO_CHECK(3, fscanf(fp," ambColor %g %g %g", &material->ambColor[0],
		  &material->ambColor[1], &material->ambColor[2]));
  IO_CHECK(3, fscanf(fp," specColor %g %g %g", &material->specColor[0],
		  &material->specColor[1], &material->specColor[2]));
  IO_CHECK(3, fscanf(fp," emisColor %g %g %g", &material->emissColor[0],
		  &material->emissColor[1], &material->emissColor[2]));
  IO_CHECK(1, fscanf(fp," shininess %g", &material->shininess));
  IO_CHECK(1, fscanf(fp," ktran %g", &material->ktran));
  fscanf(fp," }");
}
...
read_poly_setA(SceneIO *scene, FILE *fp)
{
  ...
  fscanf(fp, " {" );
  read_objectA(obj,fp);
  IO_CHECK(1,fscanf(fp," type %s",word));
  ...
  IO_CHECK(1,fscanf(fp," normType %s",word));
  ...
  IO_CHECK(1,fscanf(fp," materialBinding %s",word));
  ...
  IO_CHECK(1,fscanf(fp," hasTextureCoords %s",word));
  ...
  IO_CHECK(1,fscanf(fp," rowSize %ld", &pset->rowSize));
  IO_CHECK(1,fscanf(fp," numPolys %ld", &pset->numPolys));

  pset->poly = (PolygonIO *)calloc(pset->numPolys,sizeof(PolygonIO));
  poly = pset->poly;
  for (i = 0; i < pset->numPolys; i++, poly++) {
    fscanf(fp," poly {");
    IO_CHECK(1,fscanf(fp," numVertices %ld", &poly->numVertices));
    poly->vert = (VertexIO *)calloc(poly->numVertices,sizeof(VertexIO));
    vert = poly->vert;
    for (j = 0; j<poly->numVertices; j++, vert++) {
      IO_CHECK(3,fscanf(fp," pos %g %g %g",
	      &vert->pos[0], &vert->pos[1], &vert->pos[2])); 
      if (pset->normType == PER_VERTEX_NORMAL) {
	IO_CHECK(3,fscanf(fp," norm %g %g %g",
		&vert->norm[0], &vert->norm[1], &vert->norm[2]));
      }
      if (pset->materialBinding == PER_VERTEX_MATERIAL) {
	IO_CHECK(1,fscanf(fp," materialIndex %ld", &vert->materialIndex));
      }
      if (pset->hasTextureCoords) {
	IO_CHECK(2,fscanf(fp," s %g t %g", &vert->s, &vert->t));
      }
    }
    fscanf(fp," }");
  }
