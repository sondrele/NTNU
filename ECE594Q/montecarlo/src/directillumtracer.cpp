...
DirectIllumTracer::DirectIllumTracer(uint width, uint height, uint d)
: RayTracer(width, height, d) {

}

DirectIllumTracer::~DirectIllumTracer() {

}

SColor DirectIllumTracer::ambientLightning(float kt, SColor ka, SColor Cd) {
    return Cd.linearMult(ka).linearMult((1.0f - kt));
}

SColor DirectIllumTracer::shadeIntersection(Intersection in, int d) {
    if (d <= 0) {
        return SColor(0, 0, 0);
...
RayBuffer DirectIllumTracer::traceScene() {
        ...
        for (uint x = 0; x < WIDTH; x++) {
            ...
            for (float dn = 0; dn < numSamples; dn += 1) {
                for (float dm = 0; dm < numSamples; dm += 1) {
                    float dx = dm / (float) numSamples;
                    float dy = dn / (float) numSamples;
