...
Vertex RaySceneFactory::CreateVertexWithBindings(VertexIO &vio) {
    Vertex v = RaySceneFactory::PointToVertex(vio.pos);
    v.setMaterialIndex((uint)vio.materialIndex);
    v.setSurfaceNormal(RaySceneFactory::PointToVect(vio.norm));
...
Triangle * RaySceneFactory::CreateTriangleWithBindings(PolygonIO &pio) {
    Triangle *t = new Triangle();
    Vertex v0 = RaySceneFactory::CreateVertexWithBindings(pio.vert[0]);
    Vertex v1 = RaySceneFactory::CreateVertexWithBindings(pio.vert[1]);
    Vertex v2 = RaySceneFactory::CreateVertexWithBindings(pio.vert[2]);
    t->setA(v0);
    t->setB(v1);
    t->setC(v2);
    return t;
