...
int main() {
    ...
    kernel = buildKernel("multiply_opencl.cl", "multiply", context, device);
    ...
    cl_mem result_device = clCreateBuffer(context, CL_MEM_READ_ONLY, SIZE*sizeof(cl_float),NULL,&err);
    ...
    clEnqueueWriteBuffer(queue, a_device, CL_FALSE, 0, SIZE*sizeof(cl_float), a_host, 0, NULL, NULL);
